...
export const getWeatherByCoordinates = (lon: string, lat: string, language: string) => {
    return axios.get(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&lang=${language}&appid=${apiKey}&units=metric`);
}
